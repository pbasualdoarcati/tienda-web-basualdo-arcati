...
export const CartProvider = ({ children }) => {
    ...
    const addItem = (item, quantity) => {
        if (isInCart(item.id)) {
            const indexUpdated = product.findIndex((elem) => elem.item.id === item.id)
            product[indexUpdated].quantity = product[indexUpdated].quantity + quantity;
            setProduct( [...product])
        } else {
            const newObj = {
                item,
    ...
    const data = { clearCart, addItem, deleteItem, cartQuantity, totalPrice };
    console.log('Precio total: ',totalPrice());

    console.log('Chequeamos que producto vaya sumando la cantidad: ',product)
    return (
